...
function CssParser(opt) {
    ...
    this.opt.src = this.normalize_path(opt.src);
    this.opt.dest = this.normalize_path(opt.dest);
    this._logs = [];
}
...
CssParser.prototype = {
    ...
    calculateImagePath:function(csspath,imgpath,absolute) {
        ...
        var warn_message;
        if(!grunt.file.exists(real_full_path) && !absolute){
